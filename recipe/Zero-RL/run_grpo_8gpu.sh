#!/usr/bin/env bash
# 不用 set -u：conda activate.d/~cuda-nvcc_activate.sh 会引用可能未设置的 NVCC_PREPEND_FLAGS
set -xeo pipefail

# Zero-RL: GRPO @ 8 GPU (0-7)，与 run_grpo.sh 算法一致，仅并行度不同
...
exp_name='GRPO-Qwen3-4B-Base-8gpu'
...
# 8 卡：全分片 FSDP（与 DAPO 脚本中 fsdp_size=-1 同义）
sp_size=2
...
LOG_FILE="${LOG_DIR}/grpo_8gpu_${TIMESTAMP}.log"
CKPTS_DIR="/data3/yyy/verl/checkpoints/zero_rl/${exp_name}"

echo "========================================================="
echo " Zero-RL: GRPO (8 GPUs)"
echo "   模型: ${MODEL_PATH}"
echo "   数据: ${TRAIN_FILE}"
