#!/bin/bash
# 实验：Token-Identity Masked SD on DeepSeek-R1-Distill-Qwen-7B
# 用法：bash recipe/RLSD/run_exp_masked_sd_tokenid.sh
#
# GPU: 8x A800-80GB
# 目的：验证 token-identity mask 是否阻止 SD 的 epistemic suppression 和 OOD 退化
...
CKPT_DIR=/data3/yyy/verl/checkpoints/rlsd_exp_masked_sd_tokenid

LOG_DIR="${VERL_ROOT}/logs/rlsd"
mkdir -p "${LOG_DIR}" "${CKPT_DIR}"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
LOG_FILE="${LOG_DIR}/exp_masked_sd_tokenid_${TIMESTAMP}.log"

echo "========================================================"
echo " 实验：Token-Identity Masked SD on DS-R1-Distill-Qwen-7B"
echo "  模型: ${MODEL_PATH}"
echo "  数据: ${TRAIN_DATA}"
echo "  GPUs: ${CUDA_VISIBLE_DEVICES}"
echo "  Mask: token_identity (保护 10 个 epistemic token)"
echo "  日志: ${LOG_FILE}"
echo "  步数: 200 (overnight)"
echo "========================================================"
...
python recipe/RLSD/main_rlsd.py \
    ...
    trainer.experiment_name="masked-sd-tokenid-dsr1-7b-${TIMESTAMP}" \
