#!/bin/bash
# 实验3：OPSD-Only（纯 SD，无 GRPO），4 GPU
# 用法：bash recipe/RLSD/run_exp_opsd_only.sh
...
CKPT_DIR=/data3/yyy/verl/checkpoints/rlsd_exp_opsd_only

LOG_DIR="${VERL_ROOT}/logs/rlsd"
mkdir -p "${LOG_DIR}" "${CKPT_DIR}"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
LOG_FILE="${LOG_DIR}/exp_opsd_only_${TIMESTAMP}.log"

echo "========================================================"
echo " 实验3：OPSD-Only（纯 SD 蒸馏）"
echo "  模型: ${MODEL_PATH}"
echo "  数据: ${TRAIN_DATA}"
...
conda run -n ${CONDA_ENV} --no-capture-output \
    python recipe/RLSD/main_rlsd.py \
        ...
        trainer.experiment_name="opsd-only-qwen25-3b-${TIMESTAMP}" \
