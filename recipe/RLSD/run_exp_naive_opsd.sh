#!/bin/bash
# 实验：Naive OPSD Baseline (No Mask) on DeepSeek-R1-Distill-Qwen-7B
# 用法：bash recipe/RLSD/run_exp_naive_opsd.sh
#
# GPU: 8x A800-80GB
# 目的：复现论文 SDPO 退化 — negative control for masked OPSD
...
CKPT_DIR=/data3/yyy/verl/checkpoints/rlsd_exp_naive_opsd
...
echo "========================================================"
echo " 实验：Naive OPSD Baseline on DS-R1-Distill-Qwen-7B"
echo "  模型: ${MODEL_PATH}"
echo "  数据: ${TRAIN_DATA}"
...
python recipe/RLSD/main_rlsd.py \
    ...
    trainer.experiment_name="naive-opsd-dsr1-7b-${TIMESTAMP}" \
