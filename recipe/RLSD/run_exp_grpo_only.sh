#!/bin/bash
# 实验1：GRPO-only baseline（4 GPU）
# 用法：bash recipe/RLSD/run_exp_grpo_only.sh
...
CKPT_DIR=/data3/yyy/verl/checkpoints/rlsd_exp_grpo_only

LOG_DIR="${VERL_ROOT}/logs/rlsd"
mkdir -p "${LOG_DIR}" "${CKPT_DIR}"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
LOG_FILE="${LOG_DIR}/exp_grpo_only_${TIMESTAMP}.log"

echo "========================================================"
echo " 实验1：GRPO-only baseline"
echo "  模型: ${MODEL_PATH}"
echo "  数据: ${TRAIN_DATA}"
