#!/bin/bash
# 实验：COT Naive OPSD on DeepSeek-R1-Distill-Qwen-1.5B (8 GPUs)
# 用法：bash recipe/RLSD/run_exp_cot_naive_opsd_1.5b.sh
#
...
CKPT_DIR=/data3/yyy/verl/checkpoints/rlsd_exp_cot_naive_opsd_1.5b

LOG_DIR="${VERL_ROOT}/logs/rlsd"
mkdir -p "${LOG_DIR}" "${CKPT_DIR}"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
LOG_FILE="${LOG_DIR}/exp_cot_naive_opsd_1.5b_${TIMESTAMP}.log"

echo "========================================================"
echo " 实验：COT Naive OPSD on DS-R1-Distill-Qwen-1.5B (8 GPUs)"
echo "  模型: ${MODEL_PATH}"
echo "  数据: ${TRAIN_DATA}"
...
python recipe/RLSD/main_rlsd.py \
    ...
    trainer.experiment_name="cot-naive-opsd-dsr1-1.5b-${TIMESTAMP}" \
